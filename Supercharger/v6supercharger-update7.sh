...
do
 ...
 echo " =============================================="
 echo " 1. SuperCharger & Launcher Status for Update 7"
 ...
 if [ -f "/data/SuperChargerMinfree" ]; then
	awk -F , '{print "     Prior minfrees = "$1/256","$2/256","$3/256","$4/256","$5/256","$6/256 " mb"}' /data/SuperChargerMinfree
 fi
 echo ""
...
EOF
		...
		if [ "$opt" -le 12 ]; then
			...
			if [ "$opt" -eq 10 ]; then
				...
				echo "$SL1,$SL2,$SL3,$SL4,$SL5,$SL6" > /data/SuperChargerCustomMinfree
				echo ""
				$sleep
			fi
			echo $line
